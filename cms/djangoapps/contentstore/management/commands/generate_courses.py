    ...
    def handle(self, *args, **options):
        ...
        for course_settings in courses:
            ...
            try:
                ...
                successful_courses.append((unicode(new_course.id), new_course.display_name))
            except DuplicateCourseError:
                logger.warning("Course already exists for %s, %s, %s", org, num, run)

        # Compact log of successfully created courses
        for (course_id, display_name) in successful_courses:
            logger.info("{}, {} was created successfully in Studio".format(course_id, display_name))
